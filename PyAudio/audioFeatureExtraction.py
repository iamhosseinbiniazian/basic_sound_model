...
def mfccInitFilterBanks(fs, nfft):
    ...
    nfreqs = numpy.arange(int(nfft)) / (1. * nfft) * fs
...
def stChromagram(signal, Fs, Win, Step, PLOT=False):
    ...
    if (PLOT):
        ...
        ax.set_yticks(range(int(Ratio / 2),int(len(FreqAxis) * Ratio), int(Ratio)))
        ax.set_yticklabels(FreqAxis[::-1])
        TStep = countFrames / 3
        TimeTicks = range(0, int(countFrames), int(TStep))
...
def dirWavFeatureExtraction(dirName, mtWin, mtStep, stWin, stStep, computeBEAT=False):
    ...
    for i, wavFile in enumerate(wavFilesList):
        ...
        if computeBEAT:  # mid-term feature extraction for current file
            ...
            [MidTermFeatures, _] =         (x, Fs, round(mtWin * Fs), round(mtStep * Fs), round(Fs * stWin),
                                                       round(Fs * stStep))
