...
import sklearn.cluster
import hmmlearn
from hmmlearn import hmm as hmmmm
...
def plotSegmentationResults(flagsInd, flagsIndGT, classNames, mtStep, ONLY_EVALUATE=False):
    ...
    if not ONLY_EVALUATE:
        ...
        labelx=[i+1 for i in range(int(Duration))]
        labely=[i for i in range(len(classNames))]
        ax1.set_xticks(labelx)
        ax1.set_yticks(labely)
...
def trainHMM_fromFile(wavFile, gtFile, hmmModelName, mtWin, mtStep):
    ...
    hmm = hmmmm.GaussianHMM(startprob.shape[0], "diag")  # hmm training
...
def mtFileClassification(inputFile, modelName, modelType, plotResults=True, gtFile=""):
    '''
...
def silenceRemoval(x, Fs, stWin, stStep, smoothWindow=0.5, Weight=0.5, plot=False):
    ...
    T = (numpy.mean((1 - Weight) * ProbOnsetSorted[0:int(Nt)]) + Weight * numpy.mean(ProbOnsetSorted[-int(Nt)::]))
...
def speakerDiarization(fileName, numOfSpeakers, mtSize=2.0, mtStep=0.2, stWin=0.05, LDAdim=35, PLOT=True):
    '''
    ARGUMENTS:
