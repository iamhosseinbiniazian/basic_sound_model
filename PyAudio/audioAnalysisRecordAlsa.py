import sys, os, alsaaudio, time, audioop, numpy, glob, scipy, subprocess, wave, threading, shutil
...
def recordAnalyzeAudio(duration, outputWavFile, midTermBufferSizeSec, modelName, modelType):
    ...
    while len(allData) < duration * Fs:
        ...
        if len(midTermBuffer) == midTermBufferSize:
            count += 1
            if Classifier != None:
                [mtFeatures, stFeatures] = aF.mtFeatureExtraction(midTermBuffer, Fs, 2.0 * Fs, 2.0 * Fs, 0.020 * Fs,
                                                                  0.020 * Fs)
